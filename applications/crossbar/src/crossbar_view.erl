...
get_results(#{databases := [Db|RestDbs]=Dbs
    ...
    lager:debug("querying view '~s' from '~s', starting at '~p' with page size ~p and limit ~p in direction ~s [~p : ~p]"
               ,[View, Db, StartKey, PageSize, LimitWithLast, Direction, ChunkSize, TotalQueried]
               ),
